...
const STATS: ReadonlyArray<{
  ...
  {
    key: "total",
    field: "my_certificates",
    icon: FileText,
    toneClassName: "bg-primary-soft text-primary",
  },
  {
    key: "new",
    field: "new_certificates",
    icon: FilePlus2,
    ...
    <div>
      ...
      <ul className="mt-8 grid gap-4 sm:grid-cols-2">
        {STATS.map(({ key, field, icon: Icon, toneClassName }) => (
          <li
            key={key}
            className="border-line bg-surface-sky flex items-center gap-4 rounded-2xl border p-5"
          >
            <span
              className={cn(
            ...
            <div className="min-w-0">
              <p className="text-body text-[13.5px]">
                {t(`dashboard.profile.certificates.${key}`)}
              </p>
