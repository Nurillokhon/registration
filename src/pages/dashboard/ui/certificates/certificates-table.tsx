...
export function CertificatesTable({ certificates }: { certificates: readonly CertificateListItem[] }) {
        ...
        <table className="w-full min-w-[760px] text-left text-[14px]">
          <thead>
            <tr className="border-line text-neutral border-b text-[12px] tracking-wide uppercase">
              ...
              <th scope="col" className={HEAD_CELL_CLASS_NAME}>
                <span className="sr-only">{t('dashboard.certificates.columns.actions')}</span>
              </th>
            </tr>
          </thead>
          <tbody className="divide-line divide-y">
            {certificates.map((certificate) => (
              <tr key={certificate.id} className="hover:bg-surface-sky transition-colors">
                ...
                <td className="px-5 py-4">
                  <CertificatePaymentBadge isPaid={isCertificatePaid(certificate)} />
                </td>
