...
export function canEditCertificate(certificate: Pick<CertificateListItem, 'status' | 'is_edit'>) {
...
export function isCertificatePaid(certificate: Pick<CertificateListItem, 'is_paid'>) {
...
export function getStatusTone(status: string | null | undefined): StatusTone {
  ...
  if (normalized === 'PROBLEM' || normalized.includes('REJECT')) return 'danger'
  if (['APPROV', 'ACCEPT', 'CONFIRM', 'SUCCESS'].some((root) => normalized.includes(root))) {
    return 'success'
  }
  if (['REVIEW', 'PROCESS', 'PENDING'].some((root) => normalized.includes(root))) return 'warning'
  return 'neutral'
}
...
/** extra_data JSON satrini "maydon nomi → qiymat" ko'rinishiga keltiradi; buzilgan JSON bo'sh obyekt beradi. */
export function parseExtraData(raw: string | null | undefined): Record<string, string> {
  if (!raw) return {}
